...
describe Postcodesio do
  ...
  context 'requesting information on a single postcode works correctly' do
    ...
    before(:all) do
      ...
      UK_countries = ['England', 'Scotland', 'Wales', 'Northen Ireland']
    end
    ...
    it "should have a results hash" do
      expect(@response['result'].class).to eq Hash
    end

    it "should return a postcode between 5-7 in length"  do
      expect(@response['result']['postcode'].strip.length).to be_between(5,7)
    end
    ...
    it "should return a country which is one of the four constituent countries of the UK" do
      expect(@response['result']['country']).to include('England')
    end
  ...
  context "multiple postcodes validation" do
    ...
    before(:all) do
      @postcodesio = Postcodesio.new
      @response = @postcodesio.get_multiple_postcodes(['E50LH', 'SW1A1AA', 'SW1A2AB'])
    ...
    it "should return the first query as the first postcode in the response" do
      expect(@first_result['query']).to eq 'E50LH'
    end

    it "should return the second query as the first postcode in the response" do
      expect(@second_result['query']).to eq 'SW1A1AA'
    ...
    it "should return a primary_care_trust string" do
      expect(@second_result['result']['primary_care']).to be_a(String)
    end
    ...
    it "should return a msoa string" do
      expect(@secon_result['result']['msoa']).to be_a(String)
    end
